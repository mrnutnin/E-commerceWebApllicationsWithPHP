    ...
    <div class="body">
        ...
        <div class="container">
            <div class="account account--fixedSmall">
                ...
                <?php
                $stmt = $pdo->prepare("SELECT * FROM users WHERE Email = ? AND question = ? AND answer = ? ");
                $stmt->bindParam(1, $_POST["Email"]);
                $stmt->bindParam(2, $_POST["question"]);
                $stmt->bindParam(3, $_POST["answer"]);
                $stmt->execute();
                $row = $stmt->fetch();

                if (!empty($row)) { 
                    echo '<h2 class="page-heading">';
                    echo 'Your password is : '.$_SESSION["password"] = $row["password"];
                    echo '</h2>';
                    echo '<p style="text-align: center">The page is close in 5 second.</p>';
                    echo '<Meta http-equiv="refresh" content="5;URL=login.php">';

                } else {
                    echo '<h4 style="text-align: center; margin-top:50px;">Email or Question or Answer incorrect.</h4>';
                    echo '<p style="text-align: center; margin-top:50px;" >Please try again...</p>';
                    echo '<script type="text/javascript">setTimeout( "window.location.href = history.back(1)", 4000); </script>';
